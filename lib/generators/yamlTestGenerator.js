...
exports.init = function(context) {
  ...
  var section = context.section || "tests"
  if(_.isArray(doc[section])) {
    _tests = _.map(doc[section], function(test) { return exports.makeTestVariations(test, defaults, context) })
    _tests = _.flatten(_tests, true)
    _tests = _.filter(_tests, function(test) { return !_.has(test, "disabled") })
  } else if(!_.isEmpty(doc[section])){
    console.log("Not a valid test section (expected Array): " + section)
  }
}
