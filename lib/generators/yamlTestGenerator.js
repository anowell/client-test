...
exports.init = function(context) {
  ...
  if(_.isArray(doc[phase])) {
    ...
    logger.warn("Not a valid test phase (expected Array): " + phase)
  }
}


exports.getNextTests = function() { return _tests }
exports.hasNextTests = function() { return _tests.length > 0 }
...
var makeTest = function(test, defaults, context) {
  ...
  _.each(['tag','data', 'dataFile', 'auth','timeout', 'method'], function (prop) {
    if(typeof testDefinition[prop] !== 'undefined') {
  ...
  testDefinition.driverOptions.headers = testDefinition.driverOptions.headers || {}
  testDefinition.driverOptions.headers = _.mapKeys(testDefinition.driverOptions.headers, function(v, k) { return k.toLowerCase() })
  var d = testDefinition.driverOptions.data
  if(_.isPlainObject(d) || _.isArray(d) || _.isNumber(d) || _.isNull(d)) {
  ...
  if(context.saltQueries) {
    ...
    var separator = (testDefinition.url.indexOf('?') === -1) ? '?' : '&'
    testDefinition.url = testDefinition.url + separator + saltString
  }
