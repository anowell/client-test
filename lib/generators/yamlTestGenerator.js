...
//   - May specify HTTP method and data as {method:data}, e.g., { post : { message : "new test message" } }
//
...
var makeTest = function(test, defaults, context) {
  ...
  _.each(['get','put','patch','post','delete'], function (verb) {
    if(testDefinition[verb]) {
      ...
      testDefinition.driverOptions.data = testDefinition[verb]
      return false
    }
  })


  if(testDefinition.tag) {
    testDefinition.driverOptions.tag = testDefinition.tag
  }
