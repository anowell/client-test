      ...
      driver.run(url, runtimeDefinition.driverOptions, time, exports.loggers, function(response) {
        ...
        _.each(runtimeDefinition.assert, function(a) {
          ...
          try {
            ...
            if(e && e.message) {
              errorTrace += red("FAILED: ") + "assert " + a.check + '\n'
              errorTrace += url + '\n' + e.message + '\n'
            ...
            return false // Only allow one failed assertion per testUrl
          }
        })

        // CONSIDER: execute completion cb BEFORE running assertion (so that assertions don't impact perf)
        cb(null, response)
      })
      ...
      if( results.length > 1 ) {
        _.each(runtimeDefinition.compare, function(c){
          try {
            ...
            errorTrace += red("FAILED: ") + "compare " + c + '\n' + testUrls.join("\n") + '\n'
            if(e && e.message) {
  ...
  var enqueueTests = function(tests) {
    ...
    nextTests = _.map(nextTests, function(test) {
      ...
      return _.pick(test,
        ...
        'debug'           // OPTIONAL: boolean to indicate if runner debug output should be logged
      )
    })
